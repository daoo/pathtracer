            ...
            match node {
                ...
                KdNode::Node{plane, left, right} => {
                    ...
                    if ray.direction[axis] == 0. {
                        ...
                        let fst = if ray.direction[axis] >= 0. { left } else { right };
                        let snd = if ray.direction[axis] >= 0. { right } else { left };
                        if t >= tmax {
                            node = fst;
                        } else if t <= tmin {
                            node = snd;
...
mod tests {
    ...
    fn intersect_ray_intersecting_split_plane_and_both_triangles() {
        let triangle0 = Triangle{ v0: vector![0., 0., -1.], v1: vector![2., 0., -1.], v2: vector![2., 2., -1.] };
        let triangle1 = Triangle{ v0: vector![0., 0., 1.], v1: vector![2., 0., 1.], v2: vector![2., 2., 1.] };
        let tree = KdTree{
            root: KdNode::Node{
        ...
        assert_eq!(tree.intersect(&ray1, 0., 1.), Some(TriangleRayIntersection { t: 0.25, u: 0., v: 0.5 }));
        assert_eq!(tree.intersect(&ray2, 0., 1.), Some(TriangleRayIntersection { t: 0.25, u: 0., v: 0.5 }));
    }

    #[test]
    fn intersect_ray_parallel_to_split_plane_and_intersecting_one_triangle() {
        let triangle0 = Triangle{ v0: vector![0., 0., 0.], v1: vector![1., 0., 0.], v2: vector![0., 1., 0.] };
        let triangle1 = Triangle{ v0: vector![1., 0., 0.], v1: vector![2., 0., 0.], v2: vector![2., 1., 0.] };
        let tree = KdTree{
            root: KdNode::Node{
                plane: Aap { axis: Axis::X, distance: 1. },
        ...
        assert_eq!(tree.intersect(&ray_triangle0_v0, 0., 1.), Some(TriangleRayIntersection { t: 0.5, u: 0., v: 0. }));
        assert_eq!(tree.intersect(&ray_triangle1_v1, 0., 1.), Some(TriangleRayIntersection { t: 0.5, u: 1., v: 0. }));
    }

    #[test]
    fn intersect_ray_orthogonal_to_split_plane_and_intersecting_both_triangles() {
        let triangle0 = Triangle{ v0: vector![0., -1., -1.], v1: vector![0., 1., -1.], v2: vector![0., 1., 1.] };
        let triangle1 = Triangle{ v0: vector![2., -1., -1.], v1: vector![2., 1., -1.], v2: vector![2., 1., 1.] };
        let tree = KdTree{
            root: KdNode::Node{
                plane: Aap { axis: Axis::X, distance: 1. },
                left: Box::new(KdNode::Leaf(vec![&triangle0])),
