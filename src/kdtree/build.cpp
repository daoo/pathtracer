...
void go_sah(LinkedNode* node,
  ...
  if (depth >= 20 || cost > COST_INTERSECT * triangles.size()) {
    ...
    go_sah(node->split.left, depth + 1, next_axis(axis), left_box,
           left_triangles);
    go_sah(node->split.right, depth + 1, next_axis(axis), right_box,
           right_triangles);
  }
}
...
void go_naive(LinkedNode* node,
  ...
  if (depth >= 20 || triangles.size() <= 10) {
    ...
    go_naive(node->split.left, depth + 1, next_axis(axis), left_box,
             left_triangles);
    go_naive(node->split.right, depth + 1, next_axis(axis), right_box,
             right_triangles);
  }
}
