...
inline glm::vec3 texture_sample(const Texture& texture, float x, float y) {
  assert(x >= 0.0f && x < 1.0f);
  assert(y >= 0.0f && y < 1.0f);
  return texture_sample(texture, static_cast<unsigned int>(x * texture.width),
                        static_cast<unsigned int>(y * texture.height));
}

inline glm::vec3 texture_sample(const Texture& texture,
                                unsigned int x,
                                unsigned int y) {
...
inline glm::vec3& texture_sample(Texture& texture,
                                 unsigned int x,
                                 unsigned int y) {
  assert(x < texture.width);
  assert(y < texture.height);
